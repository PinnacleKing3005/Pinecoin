...
unsigned int nTargetSpacing = 60 * 2; // 2 mins
static const int64_t nTargetTimespan = 16 * 60; // 16 mins

unsigned int nStakeMinAge = 3 * 60 * 60; //3 h
...
const string strMessageMagic = "PINE Signed Message:\n";
...
bool CBlock::AcceptBlock()
{
    ...
    if (LIMIT_POW_BLOCKS && IsProofOfWork() && nHeight > LAST_POW_BLOCK)
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PINE", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Turkey’s Currency Crisis Tests Erdogan's Approach";
        CTransaction txNew;
        txNew.nTime = 1534201459;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce   = !fTestNet ? 495882601 : 495882601;
